...
public class Mongodb {
    ...
    public void connectMongodb(){
        ...
        ServerAddress serverAddress = new ServerAddress("192.168.109.60",27018);
        List<ServerAddress> addrs = new ArrayList<ServerAddress>();
        addrs.add(serverAddress);
        //MongoCredential.createScramSha1Credential()三个参数分别为 用户名 数据库名称 密码
        MongoCredential credential = MongoCredential.createScramSha1Credential("battery", "battery", "battery".toCharArray());
        List<MongoCredential> credentials = new ArrayList<MongoCredential>();
        credentials.add(credential);
        //通过连接认证获取MongoDB连接
        MongoClient mongoClient = new MongoClient(addrs,credentials);
        //连接到数据库
        MongoDatabase mongoDatabase = mongoClient.getDatabase("battery");
        ...
        while( mongoCursor1.hasNext() ){
            ...
            for( Map.Entry<String,Object> map : setVal ){
                ...
                if( heartVal.indexOf(",") > -1 ){
                    ...
                    System.out.print("  Status...hexToBinary..."+ ApiUtils.hexToBinaryString( new String(getLenArr[25]+getLenArr[26]+getLenArr[27]+getLenArr[28]) ));
