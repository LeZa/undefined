    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        switch (command.getType()){
            ...
            case PUT:
                ...
                /*rocksDBHolder.getResource().merge(cmd.getKey(), cmd.getValue());*/
                System.out.println( new String( cmd.getKey() ) );
                if( rocksDBHolder.getResource().get(cmd.getKey()) != null){

                    byte[] exists  = rocksDBHolder.getResource().get(cmd.getKey());

                    byte[] newVal = cmd.getValue();
                    String existsStr =  new String( exists );
                    String newStr =  new String( newVal );
                    String putStr = existsStr + ","+ newStr;
                    rocksDBHolder.getResource().put(cmd.getKey(),putStr.getBytes());
